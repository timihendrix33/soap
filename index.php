<!DOCTYPE html>
<html lang="en">
<head>
<!-- Meta, title, CSS, favicons, etc. -->
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta name="description" content="Startup Responsive Landing page Template for startup, web services, newsletter signup, lead generation etc..">
<title>Son of a Pitch Awards Havas Creative Group</title>

<!-- Bootstrap core CSS -->
<link href="css/bootstrap.css" rel="stylesheet">

<!-- Startup CSS -->
<link href="css/startup.css" rel="stylesheet">
<script src="js/modernizr.js"></script> 
<!-- HTML5 shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!--[if lt IE 9]>
  <script src="js/html5shiv.js"></script>
  <script src="js/respond.min.js"></script>
<![endif]-->

<!-- Favicons -->
<link rel="shortcut icon" href="images/favicon.png">

<!-- Custom Google Font : Open Sans -->

<link href='http://fonts.googleapis.com/css?family=Open+Sans:300,600' rel='stylesheet' type='text/css'>
<script language="javascript" type="text/javascript">
//   function resizeIframe(obj) {
//     var 
//     obj.style.height = obj.contentWindow.document.body.scrollHeight + 'px';
//   }
//   var iframe = document.getElementById("iframe");
//   resizeIframe(iframe);
</script>
</head>
<body>
  <nav id="cd-vertical-nav">
    <ul>
      <li>
        <a href="#top" data-number="1">
          <span class="cd-dot"></span>
          <span class="cd-label">Top</span>
        </a>
      </li>
      <li>
        <a href="#video" data-number="2">
          <span class="cd-dot"></span>
          <span class="cd-label">The Son of A Pitch Awards</span>
        </a>
      </li>
      <li>
        <a href="#categories" data-number="3">
          <span class="cd-dot"></span>
          <span class="cd-label">Categories</span>
        </a>
      </li>
      <li>
        <a href="#entry" data-number="4">
          <span class="cd-dot"></span>
          <span class="cd-label">Enter</span>
        </a>
      </li>
    </ul>
  </nav>
  <a class="cd-nav-trigger cd-img-replace"><span></span></a>
<div class="rotation-container">
<!-- Parallax Header Starts -->
<main id="top" class="masthead cd-section" >

    <div class="container"> 
      <div class="row">
        <div class="col-xs-4 col-md-2 col-md-offset-10">
          <a href="#"><img src="images/logo.png" alt="Son of a Pitch" /></a>
        </div>
      </div>
      <div class="row">
        <div class="col-md-8">
          <h1>We know why you got into advertising.</h1>
          <p>The big shoots. The enormous budgets. The all-expense-paid, business-class-travel, five-star shoots. Not to mention the shiny awards, and open-bar after parties.</p>
          <p>Then, of course, there’s pitching.  Sleepless. Endless. Glory-less. Not a lobster-tail hors d'oeuvre in sight.</p>
          <p>In a word- Thankless. We feel your pain. And we want to reward it. With US$5000.</p>
          <p>Have you always felt like your pitch deserved to earn you more than heartburn and caffeine shakes? Now it can.</p>
        </div>
      </div>
      <div class="row arrow-container">
        <!--  Scroll to Explore --> 
        <a href="#video" class="scrollto">
          SCROLL<br />DOWN
          <p class="scrollto--arrow"><img src="images/scroll_down.png" alt="scroll down arrow"></p>
        </a> 
        <!-- // Scroll to Explore Ends -->       
      </div>
    </div>
</main>
<!-- // Parallax Header Ends --> 
<div class="rotate clockwise cd-section" id="video">
  <div class="container rotate counter-clockwise">
    <div class="col-md-6"><img src="images/placeholder-vid.jpg" /></div>
    <div class="col-md-6">
      <h4>INTRODUCING</h4>
      <h3>The Son of A Pitch Awards</h3>
      <p>It’s a global initiative to reward everyone at HAVAS who cries, sweats, and bleeds for pitches. So even if your pitch didn’t win the business, it can still win you US$5000 for your team!</p>
      <a href="#entry" class="get-started"><button class="btn btn-default">GET STARTED</button></a>
    </div>
  </div>
</div>
<div class="rotate counter-clockwise cd-section" id="categories">
  <div class="container rotate clockwise"> 
    <div class="row">
      <h3>Six Categories to enter.<br />
      Six chances at redemption.</h3>
      <p>It’s advertising. You know the drill.</p>
    </div>
    
    <!-- Categories Section -->
    <section class="row features"> 
      <!-- Category 01 -->
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-odds.png" alt="Overcoming the odds icon">
          <div class="caption">
            <h4>Overcoming the Odds</h4>
            <p>We've all been there.  David vs goliath.  The OK Corral. Jack Bauer. Wil E. Coyote. This category acknowledges winning when the odds are stacked up against you. Kudos. You deserve more than a pat on the head and a dog biscuit. And we’re gonna give you more. </p>
          </div>
        </div>
      </div>
      
      <!-- Category 02 -->
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-odds.png" alt="Breaking new ground icon">
          <div class="caption">
            <h4>Instant Expertise</h4>
            <p>It's the ‘ol catch 22.  No entry without prior experience.  Which means you need to create that experience. Out of thin air. In other words, yes- somewhere, at some point, someone at this agency has had prior experience on that sort of business. A little embellishment? Sure. After all, it’s advertising. Which is what we do best. </p>
          </div>
        </div>
      </div>

      
      <!-- Category 03 -->
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-odds.png" alt="Most creative delivery icon">
          <div class="caption">
            <h4>Most Creative Delivery</h4>
            <p>They don't call it strategic pitch theater for nothing.  This category is for those producers and stage-theater managers who turn drama into winning.  Big or small, no one really cares. As long as it contributes to a win. Or a kick-ass portfolio piece.</p>
          </div>
        </div>
      </div>

    </section>
    <section class="row features">       
      
      <!-- Category 04 -->
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-odds.png" alt="Most Networked">
          <div class="caption">
            <h4>Most Networked</h4>
            <p>Networked. N-e-t-w-o-r-k-e-d. That’s a funny word. But this is no laughing matter. No, it’s the Son of a Pitch Awards. And sometimes, a pitch takes a village. Make that a Havas Village. And if you've networked more partners in the company than past agency presidents, this is the category for you.</p>
          </div>
        </div>
      </div>

      <!-- Category 05 -->    
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-win-got-away.png" alt="The win that got away icon">
          <div class="caption">
            <h4>The Win that Got Away</h4>
            <p>You should have won it. Everyone thought you should have won it. Hell, even the clients kinda/sorta mentioned you won it. But you didn't. Fortunately in this contest, even the losers go home winning. Tell us your story of the one that should have never gotten away, and we’ll give you 10 grand that should.</p>
          </div>
        </div>
      </div>
      <div class="col-sm-12 col-md-4">
        <div class="thumbnail"> <img src="images/icon-win-got-away.png" alt="The win that got away icon">
          <div class="caption">
            <h4>Buzzer Beater</h4>
            <p>You should have won it. Everyone thought you should have won it. Hell, even the clients kinda/sorta mentioned you won it. But you didn't. Fortunately in this contest, even the losers go home winning. Tell us your story of the one that should have never gotten away, and we’ll give you 10 grand that should.</p>
          </div>
        </div>
      </div>

    </section>  
  </div>
  <!-- // Container Ends --> 
</div>

<!-- Grey Highlight Section -->
<div class="entry rotate clockwise cd-section" id="entry">
  <div class="container rotate counter-clockwise"> 
    <div class="row">
      <div class="col-md-8">
        <h2>Does your team deserve US$5000, and then some?</h2>
        <h4>PROBABLY. BUT WE'RE ONLY GIVING AWAY 5K.</h4>
      </div>

    </div>
    <section>
    <?php 


      
      $curl = new WufooCurl();
      $response = $curl->getAuthenticated('http://havasworldwide.wufoo.com/api/v3/forms/son-of-a-pitch-awards.json', self::'9AQ3-WY7H-6H09-T1SS');
      $response = json_decode($response);
      $hash = $response->Forms[0]->Hash;
      echo $this->getFormSnippet($hash, self::'havasworldwide');

      ?>


<!--       <div id="wufoo-qlf48up0rqfdke">
      Fill out my <a href="http://havasworldwide.wufoo.com/forms/qlf48up0rqfdke">online form</a>.
      </div> -->
      <script type="text/javascript">
      // var qlf48up0rqfdke;(function(d, t) {
      // var s = d.createElement(t), options = {
      // 'userName':'havasworldwide',
      // 'formHash':'qlf48up0rqfdke',
      // 'autoResize':true,
      // 'height':'1324',
      // 'async':true,
      // 'host':'wufoo.com',
      // 'header':'hide',
      // 'ssl':false};
      // s.src = ('https:' == d.location.protocol ? 'https://' : 'http://') + 'www.wufoo.com/scripts/embed/form.js';
      // s.onload = s.onreadystatechange = function() {
      // var rs = this.readyState; if (rs) if (rs != 'complete') if (rs != 'loaded') return;
      // try { qlf48up0rqfdke = new WufooForm();qlf48up0rqfdke.initialize(options);qlf48up0rqfdke.display(); } catch (e) {}};
      // var scr = d.getElementsByTagName(t)[0], par = scr.parentNode; par.insertBefore(s, scr);
      // })(document, 'script');
      </script>

    </section>
    <section class="row breath">
      <p>Scroll back up to the top of the form if you don't see anything after submitting.</p>
    </section>
  </div>
</div>
<!-- // End Grey Highlight Section --> 

<div class="rotate counter-clockwise" id="footer">
  <div class="container rotate clockwise">
    
      <div class="col-md-4 footerlinks">
        <h5>Contact Son of a Pitch Team</h5>
        <p>If you have any questions or need clarification, send us an email at <a href="mailto:sonofapitch@havas.com">sonofapitch@havas.com</a>. We'll follow up with you.</p>
      </div>  
      <div class="col-md-4 col-md-offset-3 footerlinks"> 
        <h5>Terms & Conditions</h5>
        <p>Submissions will be accepted through to Monday 24th of November.</p>
        <p>The pitch effort need not have been successful for the entry to be a potential winner.</p>
        <p>There is no limit to the number of entries per agency.</p>
        <p>Winners will be announced January 2015 at the GLM.</p>
        <a href="terms.html">SEE FULL TERMS & CONDITIONS</a>
      </div>
    <!-- // End Client Logos --> 
    
  </div>
</div>
<div class="footer-end"></div>
</div><!-- // End Rotation Container -->


<!-- JS and analytics only. --> 
<!-- Bootstrap core JavaScript
================================================== --> 
<!-- Placed at the end of the document so the pages load faster --> 
<script src="js/jquery.js"></script> 
<script src="js/bootstrap.js"></script> 
<script src="js/easing.js"></script> 
<script src="js/vertical-fixed-nav.js"></script>
    

</body>
</html>
